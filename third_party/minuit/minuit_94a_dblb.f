...
C
     ...
     +          CVRSN*6,  COVMES(0:3)*22, CSTATU*10, CHPT*1
      ...
      DO 100  IINT= 1, NPAR
      IEXT = NEXOFI(IINT)
      CALL MNPINT(uu(IEXT),IEXT,PINTI)
      PINT(IINT) = PINTI
  100 CONTINUE
      RETURN
      END
...
C                   determine machine accuracy epsmac
      EPSTRY = 0.5
C     Replaced the minuit's implemntation of finding the machine precision with the builtin `epsilon`.
C     This shouldn't matter much anyway, because it appears no values derived from this are actually used.
...
CC
      ...
      CHARACTER*10 CPNAM
      parameter (nmcmc=100)
      character*19 char_mcmc(nmcmc),char_mcmc_acc(nmcmc)
       common/mcmc_output/nchar_mcmc,nrej_mcmc,char_mcmc,char_mcmc_acc
      COMMON
     1/MN7NAM/ CPNAM(MNE)
     2/MN7EXT/ U(MNE)     ,ALIM(MNE)  ,BLIM(MNE)
     3/MN7ERR/ ERP(MNI)   ,ERN(MNI)   ,WERR(MNI)  ,GLOBCC(MNI)
...
C                              search loop
  ...
  200       XBEST(IB) = X(IB)
            ...
            IF (ISW(5) .GE. 2) CALL MNPRIN(2,AMIN)
            ENDIF
         write(6,*) 'accepted due to chi2 improvement ',nchar_mcmc
         if(nchar_mcmc.gt.0.and.iwrite_mcmc.gt.0) then
           write(char_mcmc_acc(nchar_mcmc+1),*) nrej_mcmc+1
           write(10,999) (char_mcmc_acc(imc), imc=1,nchar_mcmc+1)
           char_mcmc_acc = char_mcmc
           nrej_mcmc = 0
           call flush(10)
         else
           iwrite_mcmc = 1
...
C                   Metropolis algorithm
         ...
         IF (BAR .LT. LOG(RNUM)) then
           write(6,*) 'rejected'
           nrej_mcmc = nrej_mcmc + 1
           GO TO 500
         endif
      ENDIF
      write(6,*) 'accepted by random chance ',nchar_mcmc
      if(nchar_mcmc.gt.0.and.iwrite_mcmc.gt.0) then
        write(char_mcmc_acc(nchar_mcmc+1),*) nrej_mcmc+1
        write(10,999) (char_mcmc_acc(imc), imc=1,nchar_mcmc+1)
        char_mcmc_acc = char_mcmc
...
CDECK  ID>, MNoptSEEK.
c==============================================================================
      SUBROUTINE MNoptSEEK(FCN,FUTIL)
C ************ DOUBLE PRECISION VERSION *************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
CC   Performs a rough (but global) minimization by monte carlo search.
CC        Each time a new minimum is found, the search area is shifted
...
CC      multiples of the nominal steps (DIRIN).
CC
      PARAMETER (MNE=100 , MNI=50, max_saved = 900000)
      PARAMETER (MNIHL=MNI*(MNI+1)/2)
      CHARACTER*10 CPNAM
     ...
     +          CVRSN*6,  COVMES(0:3)*22, CSTATU*10, CHPT*1
      ...
      if(WORD7(5).gt.0) then
        ...
        do i = 1,in_save
          ...
          do j = 1,NU
            iint = NIOFEX(j)
            if(iint.gt.0) xsave(iint,isave) = par_ext(j)
          enddo
        enddo
...
C                              search loop
  ...
  100     X(IPAR) = XMID(IPAR) + 0.5*(RNUM1+RNUM2-1.)*DIRIN(IPAR)
     ...
     &                  eval,evec)
ccc          call new_pars(NPAR,MNI,nsaved0,ISEED,X,XMID,xy,
ccc     &                  eval,evec)
        endif
      CALL MNINEX(X)
      CALL FCN(NPARX,GIN,FTRY,U,4,FUTIL)
...
CDECK  ID>, MNdoptSEEK.
c==============================================================================
      SUBROUTINE MNdoptSEEK(FCN,FUTIL)
C ************ DOUBLE PRECISION VERSION *************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
CC   Performs a rough (but global) minimization by monte carlo search.
CC        Each time a new minimum is found, the search area is shifted
...
C
     ...
     +          CVRSN*6,  COVMES(0:3)*22, CSTATU*10, CHPT*1
      ...
      PARAMETER (TWOPI=2.0d0*3.14159265358979d0)
      DIMENSION  XBEST(MNI), XMID(MNI)
      dimension xy(MNI),c2save(max_saved)
      dimension eval(MNI),evec(MNI,MNI)
      dimension par_ext(MNI)
      DATA ISEED/123456/
      data max_save/ 100 /
...
C                              search loop
      ...
      DO 500 ISTEP= 1, MXSTEP
      IF (IFAIL .GE. MXFAIL)  GO TO 600
        if(nsaved.lt.nopt_min) then
          DO 100 IPAR= 1, NPAR
          CALL MNRN15(RNUM1,ISEED)
          CALL MNRN15(RNUM2,ISEED)
  100     X(IPAR) = XMID(IPAR) + 0.5*(RNUM1+RNUM2-1.)*DIRIN(IPAR)
        else
          call new_parsd(NPAR,MNI,nsaved0,fred,ISEED,X,XMID,xy,
     &                   eval,evec)
        endif
      CALL MNINEX(X)
      CALL FCN(NPARX,GIN,FTRY,U,4,FUTIL)
...
C                    Accept new point, move there
  300 CONTINUE
      nsave = nsave + 1
      nsaved = min(nsave,max_save)
      if(nsave.gt.max_save) then
        iii = nsave/max_save
        isave = nsave - iii*max_save
        if(isave.eq.0) isave = max_save
      else
        isave = nsave
      endif
 ...
 450  continue
c     possibly reoptimise the "jump" function
      if(nsave.ge.nopt_min.and.nmissed.eq.0.and.
     &       (nopt*(nsave/nopt).eq.nsave.or.nsave.eq.nopt_min)) then
        write(6,*) 'Optimising Parameter Selection with N =',nsaved
        call paropt(NPAR,max_save,nsaved,xsave,eval,evec)
        nsaved0 = nsaved
        if(c2min.lt.c2min0+100d0) fred = 1.d0
        c2min0 = c2min
      elseif(nmissed.ge.maxmiss_list.and.nsaved.ge.nopt_min) then
        nsaved_new = max(nint(0.625d0*nsaved),nopt)
        write(6,*) 'Missed',nmissed,' consecutive tests:'
        write(6,*) 'Forget some parameter sets!'
        write(6,*) 'Dropping from Nsaved =',nsaved,
     &             ' to Nsaved =',nsaved_new
        if(naved.ge.nsaved_new+15) then
          fred_new = 1.d0
        elseif(naved.ge.nsaved_new+5) then
          fred_new = sqrt(fred)
        else
          fred_new = fred
        endif
        call forget(NPAR,max_save,nsaved,nsaved_new,c2save,xsave)
        write(6,*) 'Optimising Parameter Selection with N =',nsaved
        call paropt(NPAR,max_save,nsaved,xsave,eval,evec)
        nsave = nsaved
        nsaved0 = nsaved
        fred = fred_new
        nmissed = 0
        if(nsaved_new.ge.nopt_min) nmissed_reset = 0
      elseif(fred.gt.fred_min.and.nmissed.ge.maxmiss_fred
     &                       .and.nsaved.ge.nopt_min) then
        fred0 = fred
        fred = 0.5d0*fred
        fred = max(fred,fred_min)
        write(6,*) 'Missed',nmissed,' consecutive tests:'
        write(6,*) 'Dropping Fred from',fred0,' to',fred
        nmissed = 0
      elseif(fred.le.fred_min.and.nmissed_reset.ge.maxmiss_reset
     &                       .and.nsaved.ge.nopt_min) then
        write(6,*) 'Missed',nmissed,' consecutive tests:'
        write(6,*) 'Erase Covariance Matrix and Reset'
        nsaved = 0
        nsave = 0
        isave = 0
        nmissed = 0
...
c==============================================================================
       ...
       subroutine forget(npar,max_save,nsaved,nsaved_new,c2save,xsave)

       IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       PARAMETER (MNI=50)
       dimension c2save(max_save),xsave(MNI,max_save)

       nskip = nsaved - nsaved_new

       do 10 i=1,nsaved_new
         c2save(i) = c2save(i+nskip)
         do 10 j=1,npar
           xsave(j,i) = xsave(j,i+nskip)
 10    continue
...
c==============================================================================
       ...
       subroutine paropt(npar,max_save,nsaved,xsave,eval,evec)

       IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       PARAMETER (MNI=50)
       dimension xsave(MNI,max_save)
       dimension x_av(MNI),x_cov(MNI,MNI),eval(MNI)
       dimension evec(MNI,MNI),sa1(MNI,MNI),sa2(MNI,MNI)

       write(6,*) 'optimizing with N =',max_save,' parameter sets'
       do 10 i=1,MNI
         x_av(i)=0.d0
         do 10 j=1,MNI
           evec(j,i)=0.d0
...
c      determine the mean parameters
c      -----------------------------
       do 20 is=1,nsaved
         do 15 i=1,npar
           x_av(i) = x_av(i) + xsave(i,is)
 15      continue
 20    continue

       do 30 i=1,npar
         x_av(i) = x_av(i)/nsaved
 30    continue

c      determine the correlation matrix
c      --------------------------------
       do 50 is=1,nsaved
         do 40 i=1,npar
           do 40 j=1,i
             x_cov(j,i) = x_cov(j,i)
     &              + (xsave(j,is)-x_av(j))*(xsave(i,is)-x_av(i))
 40      continue
 50    continue

       do 60 i=1,npar
         do 60 j=1,i
           x_cov(j,i) = x_cov(j,i)/nsaved
           x_cov(i,j) = x_cov(j,i)
 60    continue

c      diagonalize
c      -----------
       call rs(MNI,npar,x_cov,eval,1,evec,sa1,sa2,ierr)
...
c      -----------------
       do i=1,npar
         do j=1,i
           dot = 0.d0
           do k=1,npar
             dot = dot + evec(k,i)*evec(k,j)
           enddo
           write(6,*) j,i,dot,evec(j,i),evec(i,j)
         enddo
       enddo
       do i=1,npar
         write(6,*) i,eval(i)
...
c==============================================================================
       ...
       subroutine new_pars(npar,MNI,nsaved0,covfac,iseed,x,xmid,y,
     &            eval,evec)
ccc       subroutine new_pars(npar,MNI,nsaved0,iseed,x,xmid,y,
ccc     &            eval,evec)
       use stdlib_stats_distribution_normal, only: rvs_normal
       IMPLICIT DOUBLE PRECISION (A-H,O-Z)
       dimension x(npar),xmid(npar),y(npar)
       dimension eval(npar),evec(MNI,MNI)
       dimension xdum(30)
       data epsilon/1.d-18/
       data iseed0 /922826/
       data iset /0/
       ...
       fac = 1.d0+1.d0/sqrt(dfloat(nsaved0))
       fac = fac*covfac

c      fill the array of principal component variables
c      -----------------------------------------------
       do 10 i=1,npar
         y(i) = fac*sqrt(eval(i)+epsilon)*rvs_normal()
         x(i) = 0.d0
         xdum(i) = 0.d0
 10    continue

c      convert back to original coordinates
c      ------------------------------------
       do 30 i=1,npar
         do 20 j=1,npar
           x(i) = x(i) + evec(i,j)*y(j)
           xdum(i) = xdum(i) + evec(j,i)*y(j)
 20      continue
         x(i) = x(i) + xmid(i)
         xdum(i) = xdum(i) + xmid(i)
 30    continue
...
c==============================================================================
       ...
       subroutine new_parsd(npar,MNI,nsaved0,fred,iseed,x,xmid,y,
     &                      eval,evec)
       ...
       fac = fred*(1.0d0+2.d0/sqrt(dfloat(nsaved0)))

c      fill the array of principal component variables
c      -----------------------------------------------
       do 10 i=1,npar
ccc         y(i) = fac*sqrt(eval(i)+epsilon)*gasdev(iseed)
         eigenval = max(eval(i),epsilon)
         y(i) = fac*sqrt(eigenval)*rvs_normal()
         x(i) = 0.d0
         xdum(i) = 0.d0
 10    continue
